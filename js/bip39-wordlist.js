...
(function(global) {
  ...
  /**
   * Busca el índice de una palabra en la wordlist
   * @param {string} word
   * @returns {number} índice (0-2047) o -1 si no existe
   */
  function getWordIndex(word) {
    return BIP39_WORDLIST.indexOf(word.toLowerCase().trim());
  }

  /**
   * Obtiene la palabra dado un índice
   * @param {number} index (0-2047)
   * @returns {string}
   */
  ...
  /**
   * Verifica si una palabra está en la wordlist
   * @param {string} word
   * @returns {boolean}
   */
  ...
  /**
   * Busca palabras que empiezan con un prefijo (para autocompletado)
   * @param {string} prefix
   * @param {number} limit
   * @returns {string[]}
   */
  function findWordsStartingWith(prefix, limit = 10) {
