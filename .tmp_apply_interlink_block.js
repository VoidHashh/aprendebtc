...
if (!blockNum || blockNum < 1 || blockNum > 6) throw new Error('Uso: node .tmp_apply_interlink_block.js <bloque> [--apply]');
const APPLY = process.argv.includes('--apply');
const BLOCK_NAME = `Bloque ${blockNum}`;
const NEXT_BLOCK = blockNum < 6 ? `## Bloque ${blockNum + 1}` : null;
const RESULT_FILE = `.tmp_block${blockNum}_apply_results.json`;
...
function parseBlockRows() {
  ...
  const end = NEXT_BLOCK ? text.indexOf(NEXT_BLOCK, start) : -1;
  ...
  for (const line of block.split(/\r?\n/)) {
    ...
    rows.push({ idx: Number(parts[0]), origin: parts[1], text: parts[2], anchor: parts[3], dest: parts[4], note: parts[5] || '' });
  }
  return rows;
}
...
function replaceFirstValid(html, row) {
  ...
  while (i < html.length) {
    if (html[i] === '<') {
      ...
      if (tag.type === 'start') {
        ...
        for (let s = stack.length - 1; s >= 0; s--) {
          if (stack[s].tag === tag.tag) { stack.splice(s, 1); break; }
        }
    ...
    if (!searchable) { out += chunk; i = end; continue; }
    const match = termRe.exec(chunk);
    if (!match) { out += chunk; i = end; continue; }
    found = true;
    if (inLink(stack)) { out += chunk; skipExistingLink = true; i = end; continue; }
...
function main() {
  ...
  const summary = {
    ...
    skippedBreakdown: skipped.reduce((acc, row) => { acc[row.reason] = (acc[row.reason] || 0) + 1; return acc; }, {}),
    results,
